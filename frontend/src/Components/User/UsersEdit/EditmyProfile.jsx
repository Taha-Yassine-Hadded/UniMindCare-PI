...
const authHeader = () => {
  ...
  return { 
    ...
    'X-Auth-FaceID': 'true' // Ajouter cet en-tête comme dans PrivateRoute.jsx
  };
};
...
useEffect(() => {
  // Fonction pour synchroniser le token entre localStorage et sessionStorage
  const syncAuthState = () => {
    ...
    // Si le token est présent dans l'un mais pas dans l'autre, synchroniser
  ...
  if (!isLoggedIn) {
    swal("Session expirée", "Veuillez vous reconnecter", "warning")
      .then(() => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.log("Token utilisé pour la mise à jour du profil:", token.substring(0, 20) + "...");
      ...
      if (!response.ok) {
        ...
        console.error("Erreur HTTP lors de la mise à jour:", response.status, errorText);
      ...
      // Si l'erreur contient "jwt", c'est probablement un problème d'authentification
      if (error.message.includes('jwt')) {
        swal("Erreur d'authentification", "Votre session a expiré. Veuillez vous reconnecter.", "error")
          .then(() => {
...
const handleEnable2FA = async () => {
  try {
    ...
    // Vérifier si le code est bien saisi
